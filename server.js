    ...
    <style>
        * {
        ...
        .hero {
            position: relative;
            height: 70vh;
            background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), var(--secondary-bg);
            display: flex;
            align-items: center;
...
<body>
    ...
    <nav class="navbar">
        <div class="nav-container">
            <div class="nav-logo">
                <span class="bb-logo">BB</span>
                <span class="movies-text">MOVIES</span>
            </div>
            <ul class="nav-menu">
        ...
        class BBMovies {
            ...
            async fetchMovies(query = '', page = 1) {
                try {
                    ...
                    const apiUrl = \`/api/movies?query=\${encodeURIComponent(query)}&page=\${page}\`;
            ...
            async loadTrendingMovies() {
                // Load latest movies for trending section
                const data = await this.fetchMovies('popular');
                const movies = data.results || data;
            ...
            createMovieCard(movie) {
                ...
                const posterUrl = movie.poster_path || movie.backdrop_path || movie.poster;
                ...
                return \`
                    <div class="movie-card" data-movie-id="\${movie.id}">
                        ...
                        <div class="movie-info">
                            ...
                            <p class="movie-description">\${overview.substring(0, 150)}...</p>
            ...
            showMovieDetails(movie) {
                ...
                modalBody.innerHTML = this.createModalContent(movie);
                ...
                return \`
                    ...
                    <div class="modal-details">
                        ...
                        <div class="modal-meta">
                            ...
                            <span>🎭 \${movie.genres ? movie.genres.map(g => g.name).join(', ') : (movie.genre_ids ? 'Various' : 'Movie')}</span>
                        </div>
                        ...
                        <div class="modal-actions">
                            ...
                            <button class="btn btn-download" onclick="app.downloadMovie(\${JSON.stringify(movie).replace(/"/g, '&quot;')})">
            ...
            async watchMovie(movie) {
                // In a real app, this would stream the movie
                // For demo, we'll simulate watching and track progress
                const existing = this.continueWatching.find(m => m.id === movie.id);
                ...
                if (!existing) {
                    ...
                    if (this.continueWatching.length > 10) {
                        this.continueWatching.pop();
                    }
            ...
            async downloadMovie(movie) {
                try {
                    ...
                    const downloadItem = {
                        ...
                        thumbnail: movie.poster_path || movie.backdrop_path || movie.poster,
                    ...
                    alert(\`Successfully downloaded: \${movie.title || movie.name}\`);
            ...
            toggleWatchlist(movie) {
                ...
                this.updateUI(); // Update watchlist buttons throughout the app
            }
                ...
                container.innerHTML = this.downloads.map(download => \`
                    <div class="download-item">
                        ...
                        <div class="download-info">
                            ...
                            <p>Size: \${download.fileSize} • Downloaded: \${new Date(download.downloadDate).toLocaleDateString()}</p>
                        </div>
            ...
            async loadMoreMovies() {
                ...
                if (searchQuery) {
                    ...
                    if (movies && movies.length > 0) {
                        this.appendMovies(movies, 'search-results-grid');
                    }
                } else {
                    const data = await this.fetchMovies('popular', this.currentPage);
                    const movies = data.results || data;
                    if (movies && movies.length > 0) {
                        this.appendMovies(movies, 'trending-grid');
                    }
                }
...
app.get('/api/movies', async (req, res) => {
    try {
        ...
        console.log('API Response received, number of results:', Array.isArray(data) ? data.length : (data.results ? data.results.length : 'unknown'));
...
app.get('/health', (req, res) => {
    res.json({ status: 'OK', message: 'BB Movies server is running' });
});
