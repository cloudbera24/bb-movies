    ...
    <script>
        ...
        async function handleYouTubeSearch() {
            try {
                ...
                youtubeResults.innerHTML = '<div class="loading"><div class="loading-spinner"></div>Searching YouTube...</div>';
        ...
        async function downloadYouTubeVideo(videoId, title, format) {
            try {
                ...
                progressText.textContent = 'Downloading "' + title + '" - ' + format.toUpperCase();
                ...
                if ('Notification' in window && Notification.permission === 'granted') {
                    new Notification('Beraflix Download Complete', {
                        body: '"' + title + '" - ' + format.toUpperCase() + ' has been downloaded',
                        icon: '/icon-192.png'
                    });
                }
...
app.get('/api/youtube/download/mp3', async (req, res) => {
  try {
    ...
    const downloadUrl = `${YOUTUBE_API_BASE}/download/ytmp3?apikey=gifted&url=${encodeURIComponent(url)}`;
    ...
    console.error('Error getting YouTube MP3 download:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to get MP3 download URL' 
    });
  }
});
...
app.get('/api/youtube/download/mp4', async (req, res) => {
  try {
    ...
    const downloadUrl = `${YOUTUBE_API_BASE}/download/ytmp4?apikey=gifted&url=${encodeURIComponent(url)}`;
    ...
    console.error('Error getting YouTube MP4 download:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to get MP4 download URL' 
    });
  }
});
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    features: ['HD Streaming', 'Offline Downloads', '4K Content', 'YouTube Downloader', 'Premium Experience', 'Multiple Categories', 'PWA Support', 'Mobile Friendly', 'K-Drama', 'Bollywood', 'Sci-Fi']
  });
});
